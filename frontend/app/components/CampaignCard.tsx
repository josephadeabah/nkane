      ...
      <Link to="/campaign" className="block flex-1">
        <div className="relative aspect-[3/2] overflow-hidden">
          ...
          <button 
            className={cn(
              ...
              isLiked ? "bg-primary/20 text-primary" : "bg-background/80 text-muted-foreground hover:text-primary"
            )}
            onClick={(e) => {
              e.preventDefault();
              setIsLiked(!isLiked);
        ...
        <div className="p-5 flex-1 flex flex-col">
          <div className="mb-3 flex items-center gap-2">
            ...
            />
            <span className="text-xs text-muted-foreground">{campaign.creator.name}</span>
          </div>
          ...
          <div className="mt-auto">
            ...
            <div className="flex justify-between text-sm mb-4">
              ...
              <span className="text-muted-foreground">${campaign.current.toLocaleString()} raised</span>
            </div>
            
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-1.5">
                <Award className="h-4 w-4 text-primary" />
                <span className="text-xs font-medium">{campaign.rewards.length} rewards</span>
              </div>
